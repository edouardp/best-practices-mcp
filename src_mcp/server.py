...
MAX_SNIPPET_SIZE = 400  # Max chars in content snippet
    ...
    return [
        {
            ...
            "end_line": r["end_line"],
            "content": r["content"][:MAX_SNIPPET_SIZE] + "..."
            if len(r["content"]) > MAX_SNIPPET_SIZE
            else r["content"],
...
def pqsoft_read_docs(documentation_path: str, start_line: int, end_line: int) -> str:
    ...
    if not documentation_path.endswith(".md"):
        result = "Error: Only .md files are allowed"
    elif any(part.startswith(".") for part in documentation_path.split("/")):
        result = "Error: Hidden files and directories are not allowed"
    else:
        docs_dir = Path("docs")
        ...
        if not result and not file_path.exists():
            result = f"File not found: {documentation_path}"
        ...
        if not result:
            try:
                ...
                result = f"Error reading file: {e!s}"
...
def pqsoft_recommend_docs(title: str) -> list[dict]:
    ...
    current_results = conn.execute(
        "SELECT content, embedding FROM documents WHERE title LIKE ? LIMIT 1", (f"%{title}%",)
    ).fetchone()

    if not current_results:
        return []

    current_embedding = current_results
    ...
    results = conn.execute(
        ...
        LIMIT 5
    """,
        (current_embedding, f"%{title}%"),
    ).fetchall()

    return [
        {
            "title": r[0],
            "context": r[1][:MAX_SNIPPET_SIZE] + "..." if len(r[1]) > MAX_SNIPPET_SIZE else r[1],
